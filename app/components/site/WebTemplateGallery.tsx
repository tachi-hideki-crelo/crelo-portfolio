...
export default function WebTemplateGallery({ config = webTemplateGallery }: { config?: WebTemplateGalleryConfig }) {
  ...
  const handlePointerDown = useCallback((event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    // A new press is a new intent; only the click generated by the previous
    // drag may be suppressed, never this press on a card.
    clearClickSuppression();
    if (event.target instanceof Element && event.target.closest('[data-template-action]')) {
      const field = fieldRef.current;
      // Stop residual wheel/drag inertia so the pressed card cannot move out
      // from under the pointer before the browser dispatches click.
  ...
  }, [clearClickSuppression, reduceMotion]);
  ...
  const finishPointer = useCallback((event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    const didDrag = pointer.didDrag || hasExceededDragThreshold(
      pointer.startX,
      pointer.startY,
      event.clientX,
      event.clientY,
  ...
  const handleCardFocus = useCallback((event: FocusEvent<HTMLElement>, index: number) => {
    // Pointer focus must leave the card in place until its click opens detail.
    // Keyboard focus can still bring an off-screen card into view.
